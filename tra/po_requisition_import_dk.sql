...
--                END IF;
    ...
    --                        TYPE_LOOKUP_CODE = REC_HDR.TYPE_LOOKUP_CODE,
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        fnd_file.put_line(fnd_file.log, 'ERROR IN UPDATING HEADER VALIDATIONS: ' || sqlerrm);
    ...
    EXCEPTION
        WHEN OTHERS THEN
            fnd_file.put_line(fnd_file.log, 'ERROR IN VALIDATIONS: ' || sqlerrm);
    END validate2;
    ...
    -- Loop through each header record
        FOR rec_hdr IN cur_hdr LOOP
            ...
            -- Validate authorization status
                rec_hdr.auth_status_CODE := get_lookup_code(rec_hdr.req_status, 'AUTHORIZATION STATUS');
                ...
                FOR rec_line IN cur_line(rec_hdr.header_id, rec_hdr.org_id) LOOP
                    IF rec_line.header_id = rec_hdr.record_id THEN
                        ...
                        -- Validate line fields
                            rec_line.org_id := valid_org(rec_line.operating_unit);
...
--                        END IF;
                        ...
                        -- Validate item ID
                            rec_line.item_id := valid_item(rec_line.item_num, rec_line.DEST_ORG_ID);
                        ...
                        -- Validate category
                            rec_line.category_id := valid_category(rec_line.category);
                        ...
                        -- Validate vendor and vendor site
                            ...
                            rec_line.vendor_site_id := valid_supplier(rec_line.vendor_site);
                            IF rec_line.vendor_site_id IS NULL THEN
                                rec_line.status := 'E';
							...
							--LOCATION_ID
							rec_line.location_id := get_line_location_id (rec_line.location_code);
							IF rec_line.line_location_id IS null
							THEN
								rec_line.status := 'E';
							    rec_line.error_message := rec_line.error_message || ' | ' || 'Location is not Valid';
							  END IF;
							
							-- DESTINATIOIN CODE
							rec_line.DESTINATION_TYPE_CODE := get_lookup_code (rec_line.destination_type,'DESTINATION TYPE');
							IF rec_line.DESTINATION_TYPE_CODE IS NULL
							  THEN
								 rec_line.error_message :=
									   rec_line.error_message
									|| ' | '
							...
							IF rec_line.source_type_code IS NULL
							  THEN
								 rec_line.status := 'E';
								 rec_line.error_message :=
									   rec_line.error_message
							...
							IF rec_dist.charge_account_id IS NULL
							THEN
							   rec_dist.status := 'E';
							...
							IF REC_LINE.STATUS = 'E' THEN
								REC_HDR.STATUS = 'E';
								REC_HDR.error_message := REC_HDR.error_message || ', ERROR IN LINE : ' || REC_LINE.error_message;
							END IF;
							
                        -- Update the line with new validated values
                            UPDATE xxqgen_po_req_line_stg_dk
                            SET
                                org_id = rec_line.organization_id,
                                DEST_ORG_ID = rec_line.destination_organization_id,
                                item_id = rec_line.item_id,
                                category_id = rec_line.category_id,
                                vendor_id = rec_line.vendor_id,
                                vendor_site_id = rec_line.vendor_site_id,
                                status = rec_line.status,
                        ...
                        EXCEPTION
                            WHEN OTHERS THEN
								...
								SET STATUS = rec_line.status,
									error_message = rec_line.error_message := 'Error IN VALIDATION : '
                                                          || sqlcode
                                                          || ' - '
                                                          || sqlerrm;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                dbms_output.put_line('ERROR IN HEADER OPERATING UNIT VALIDATE: ' || sqlerrm);
        END;
