...
BEGIN
    ...
    -- Debugging: Check if the cursor is returning any rows
    DECLARE
        LV_ROW_COUNT NUMBER;
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE('Number of records to process: ' || LV_ROW_COUNT);
    END;

    -- Process each record from the cursor
    FOR REC_HEAD IN CUR_HEAD LOOP
        -- Set default status and error message
        REC_HEAD.STATUS := 'V';
        ...
        -- Debugging: Log the values just before update
        DBMS_OUTPUT.PUT_LINE('Updating Record ID: ' || REC_HEAD.RECORD_ID);
        DBMS_OUTPUT.PUT_LINE('New STATUS: ' || REC_HEAD.STATUS || ' ERROR_MESSAGE: ' || REC_HEAD.ERROR_MESSAGE);
        ...
        IF SQL%ROWCOUNT = 0 THEN
            DBMS_OUTPUT.PUT_LINE('No rows updated for Record ID: ' || REC_HEAD.RECORD_ID);
        ELSE
            DBMS_OUTPUT.PUT_LINE('Updated Record ID: ' || REC_HEAD.RECORD_ID);
        END IF;
    END LOOP;

    DBMS_OUTPUT.PUT_LINE('HEADER VALIDATION END');

    -- Optional: Commit changes if needed (depending on your environment)
    -- COMMIT;
