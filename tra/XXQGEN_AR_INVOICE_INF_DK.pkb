/* Formatted on 5/30/2025 2:41:18 PM (QP5 v5.163.1008.3004) */
CREATE OR REPLACE PACKAGE BODY XXQGEN_AR_INVOICE_INF_DK
IS
   /***************************************************************************************************
   ...
   * =======================================================================
   * DKUMAR                            1.0                     21-MAY-2025             Initial Version
   * DKUMAR                           1.1                     22-MAY-2025             ADDING VALIDATION REQUIRED
   * DKUMAR                          1.2                     23-MAY-2025              ADDING FUNCTION FOR VALIDATION
   * DKUMAR                          1.3                     26-MAY-2025              ADDING IMPORT PROCEDURE
   ***************************************************************************************************/
   ...
   * WHO              Version #   WHEN            WHAT
   * ======================================================================
   * DKUMAR           1.0         23-MAY-2025     Initial Version
   ***************************************************************************************************/
   ...
   ***************************************************************************************************/
   ...
   BEGIN
      SELECT CUST_TRX_TYPE_ID
       ...
       WHERE UPPER (NAME) = UPPER (P_TYPE_NAME) AND ORG_ID = 204;

      --         SELECT *
      --        FROM RA_CUST_TRX_TYPES_ALL
      --       WHERE UPPER (NAME) = UPPER ('INVOICE')
      --            AND ORG_ID = 204;
      --      DEBUG
      --      FND_FILE.PUT_LINE (FND_FILE.LOG, 'ENTER IN CHK_TYPE');
   ...
   BEGIN
      ...
      --      DEBUG
      --      FND_FILE.PUT_LINE (FND_FILE.LOG, 'ENTER IN CHK_CODE_COMBINATION');
    ...
    * WHO                Version #   WHEN              WHAT
    * ======================================================================
    * DKUMAR           1.0           26-MAY-2025     Initial Version
    ***************************************************************************************************/
   ...
   IS
      ...
      IS
         SELECT *
          ...
          WHERE status = 'V' AND request_id = gn_request_id;
   ...
   BEGIN
      ...
      LOOP
         BEGIN
            ...
            SELECT xxqgen_ar_inv_hdr_id.NEXTVAL INTO LV_HDR_ID FROM DUAL;
            ...
            LOOP
               BEGIN
                  ...
                  FND_FILE.PUT_LINE (
                     FND_fILE.LOG,
                     'HEADER_ID, LINE_ID, LINE_NUMBER, INVENTORY_ITEM_ID, DESC, QUANTITY_INV, SELLING PRICE, UOM, LINE_TYPE');
                  fnd_file.put_line (
                     fnd_file.LOG,
                        l_trx_lines_tbl (l_line_cnt).trx_header_id
                     || ', '
            ...
            ELSE
               UPDATE ar_inv_header_dk
                  SET status = 'E', error_message = l_msg_data
                WHERE trx_number = rec.trx_number;

               UPDATE ar_inv_line_dk
                  SET status = 'E', error_message = l_return_status
                WHERE trx_header_id = rec.trx_number;
   ...
   * WHO                Version #   WHEN              WHAT
   * ======================================================================
   * DKUMAR           1.0           22-MAY-2025     Initial Version
   ***************************************************************************************************/
   ...
   IS
      ...
      IS
         SELECT *
           FROM AR_INV_HEADER_DK
          WHERE 1 = 1 AND STATUS = 'N' AND REQUEST_ID = GN_REQUEST_ID;

      CURSOR CUR_LINE
      IS
         SELECT *
           FROM AR_INV_LINE_DK
          WHERE 1 = 1 AND REQUEST_ID = GN_REQUEST_ID AND STATUS = 'N';
   ...
   BEGIN
      ...
      UPDATE AR_INV_HEADER_DK
         SET STATUS = 'E', ERROR_MESSAGE = 'NO LINE '
       WHERE TRX_NUMBER = (SELECT TRX_NUMBER
                             FROM AR_INV_HEADER_DK
                            WHERE TRX_NUMBER NOT IN (SELECT TRX_HEADER_ID
                                                       FROM AR_INV_LINE_DK
                                                      WHERE STATUS = 'V'));

      -- CHECK IF ANY LINE IS IN ERROR THEN ALL LINE IS IN ERROR
      UPDATE AR_INV_LINE_DK
         SET STATUS = 'E', ERROR_MESSAGE = 'ERROR IN LINE'
   ...
   ***************************************************************************************************/
   ...
   BEGIN
      ...
      UPDATE AR_INV_HEADER_DK
       ...
       WHERE     TRX_LEGAL_ENTITY IS NULL
             AND REQUEST_ID = GN_REQUEST_ID
             AND status = 'N';
      ...
      UPDATE AR_INV_HEADER_DK
       ...
       WHERE     bill_to_customer_name IS NULL
             AND REQUEST_ID = GN_REQUEST_ID
             AND status = 'N';
      ...
      UPDATE AR_INV_HEADER_DK
       ...
       WHERE     bill_to_address1 IS NULL
             AND REQUEST_ID = GN_REQUEST_ID
             AND status = 'N';
      ...
      UPDATE AR_INV_LINE_DK
         SET STATUS = 'E', ERROR_MESSAGE = ERROR_MESSAGE || ' DATE IS NULL'
       WHERE     unit_standard_price IS NULL
             AND REQUEST_ID = GN_REQUEST_ID
             AND status = 'N';
      ...
      UPDATE AR_INV_HEADER_DK
         SET STATUS = 'E', ERROR_MESSAGE = ERROR_MESSAGE || ' DATE IS NULL'
       WHERE     primary_salesrep_name IS NULL
             AND REQUEST_ID = GN_REQUEST_ID
             AND status = 'N';
      ...
      UPDATE ar_inv_line_dk LINE
         SET status = 'E',
             ERROR_MESSAGE =
                ERROR_MESSAGE || 'THERE IS NOT HEADER FOR THIS LINE'
       WHERE NOT EXISTS
                (SELECT 1
                   FROM AR_INV_HEADER_DK HDR
                  WHERE HDR.TRX_NUMBER = LINE.TRX_HEADER_ID);

      -- invoice quantity not more than quantity ordered
      UPDATE ar_inv_line_dk
         SET status = 'E',
             error_message =
                error_message
                || 'quantity invoice exceeds more than quantity ordered '
       WHERE quantity_invoiced > quantity_ordered AND status = 'E';
       
       -- trx_number
      UPDATE ar_inv_header_dk
         SET status = 'Z', error_message = error_message || 'null record!'
       WHERE     trx_number IS NULL
             AND trx_currency IS NULL
             AND bill_to_customer_name IS NULL
             AND ship_to_customer_name IS NULL
             AND term_id IS NULL
             AND primary_salesrep_name IS NULL;
   ...
   * Language     : PL/SQL
   * Description  : LOAD_DATA process to load data in staging table using sql loader.
   * History      :
   *
   * WHO                Version #   WHEN              WHAT
   * ======================================================================
   ...
   ***************************************************************************************************/
   ...
   IS
      ...
      lc_approval_status   VARCHAR2 (100) := 'INC OMPLETE';
      ln_request           NUMBER;
   ...
   BEGIN
      --        DEFINE SEQUENCE
      SELECT XXQGEN_AR_INV_BATCH_ID.NEXTVAL INTO DEF_CUR FROM DUAL;

      SELECT XXQGEN_AR_INV_BATCH_ID.CURRVAL INTO STR_CUR FROM DUAL;
      ...
      ln_request_id :=
         fnd_request.submit_request (
            application   => 'AR',
            program       => 'XXQGEN_AR_INV_DATA_LOADER',
            argument1     => 'LOAD_DATA');
      ...
      EXCEPTION
         ...
         THEN
            FND_FILE.PUT_LINE (
               FND_fILE.LOG,
                  'ERROR IN UPDATING IN WHO COLUMN : '
               || SQLCODE
               || ' - '
               || SQLERRM);
      END;
   ...
   EXCEPTION
      ...
      THEN
         FND_FILE.PUT_LINE (
            FND_FILE.LOG,
            'ERROR IN LOAD_DATA : ' || SQLCODE || ' - ' || SQLERRM);
   END LOAD_DATA;
