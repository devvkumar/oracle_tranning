   /*
       ...
       -- Function to validate Operating Unit
       ...
       -- Function to validate Requestor ID
       ...
       BEGIN
          ...
          INTO LV_CAT_ID
          FROM MTL_CATEGORIES
          WHERE SEGMENT1 || SEGMENT2 = CATEGORY;
       ...
       FUNCTION VALID_SITE(SITE VARCHAR2) RETURN NUMBER IS
          LV_SITE_ID NUMBER;
       BEGIN
          SELECT VENDOR_SITE_ID
          INTO LV_SITE_ID
          FROM AP_SUPPLIER_SITES_ALL
          WHERE VENDOR_SITE_CODE = SITE;
...
/*
    ...
    * Language     : PL/SQL     
    * Description  : Main process to insert employee details.
    * History      : 
    *
    * WHO              Version #   WHEN            WHAT
    * ===============  =========   =============   ====================================================
    ...
    ***************************************************************************************************/
          ...
          FOR REC_HEAD IN CUR_HEAD LOOP
            BEGIN
                 ...
                 END IF;
                  ...
                  LV_MESSAGE_ERROR := SQLCODE || ' - ' || SQLERRM;
       ...
       -- Procedure to validate requisition header and line data
       ...
       -- Procedure to load data into staging tables
       ...
       BEGIN
          ...
          INSERT INTO XXQGEN_PO_REQ_HDR_STG_DK (
             ...
             'Vision Operations', 1000001, 15567, 'Purchase Requisition', 'Dorat, Mrs. Melanie Celeste',
          ...
          INSERT INTO XXQGEN_PO_REQ_LINE_STG_DK (
             ...
             DESCREPTION, UOM, QUANTITY, PRICE, NEED_BY, AMOUNT, CHARGE_ACCOUNT,
             DESTINATION_TYPE, REQUESTER, REQUESTER_ID, ORGANIZATION, ORG_ID,
             LOCATION, LOCATION_ID, SOURCE, SUPPLIER, SITE, CONTACT, PHONE, STATUS,
             ERROR_MESSAGE, CREATION_DATE, CREATED_BY, LAST_UPDATED_BY, UPDATED_BY,
             REQUEST_ID, BATCH_ID
          ) VALUES (
             XXQGEN_PO_REQ_LINE_ID.NEXTVAL, XXQGEN_PO_REQ_HEADER_ID.CURRVAL, 1, 'IR0011', NULL, NULL, '', NULL,
             '', '', 5, 100, NULL, NULL, NULL, 'EXPENSE', 'Stock, Ms. Pat', '', 'Vision Operations', NULL,
             'V1- New York City', '', 'Supplier', '', '', '', NULL, 'N', '', SYSDATE, FND_PROFILE.VALUE('USER_ID'),
